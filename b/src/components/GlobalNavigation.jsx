...
const companySubmenu = [
  ...
  { label: "Qualified Agency", slug: "certification" },
  { label: "R&D Achievement", slug: "facility" },
  { label: "Global Network", slug: "global-network" },
];

const productSubmenu = [
  { label: "Maintaining Traceability", slug: "cover" },
  { label: "Standard Gas", slug: "product01" },
  { label: "Mixed Gas", slug: "mixed-gas" },
  { label: "Gas Cylinder/Valve", slug: "manufacturing-process" },
  { label: "rigas | ONE Series", slug: "one-series" },
  { label: "rigas | PAS", slug: "pas" },
  { label: "RIGAS Regulator for Calibration Gases", slug: "regulator" },
];

const isSubmenuActive = (item, pageSlug1, pageSlug2, section) =>
  pageSlug1 === section && pageSlug2 === item.slug;
...
export default function GlobalNavigation() {
  ...
  const isProductCover = pageSlug1 === "product" && pageSlug2 === "cover";
    ...
    <div className="pointer-events-none absolute left-0 top-0 z-[9999] h-full w-full">
      {showHeader && (
        <div className={twMerge("rigas-header-wrap pointer-events-auto", isIndexPage && "rigas-header-wrap--index")}>
              ...
              <ul>
                {companySubmenu.map((item) => (
                  <li key={item.label}>
                    <span
                      className={twMerge(
                        isSubmenuActive(item, pageSlug1, pageSlug2, "company") && "active",
                      )}
                    >
                      {item.label}
                    </span>
          ...
          {isProductSection && !isProductCover && (
              ...
              <ul>
                {productSubmenu.map((item) => (
                  <li key={item.label}>
                    <span
                      className={twMerge(
                        isSubmenuActive(item, pageSlug1, pageSlug2, "product") && "active",
                      )}
                    >
                      {item.label}
                    </span>
      ...
      {!((pageSlug1 === "intro") || pageSlug1 === "last" || pageSlug1 === "index") && (
        <div className={twMerge("paging-box flex items-center w-[272px] *:pointer-events-auto", pageSlug1 !== "intro" && "paging-bg")}>
          <CatalogPagingButton
            direction="prev"
            className={twMerge("paging-prev", pageSlug1 !== "intro" && "disabled:opacity-50", pageSlug1 === "intro" && "opacity-0")}
          >
          ...
          <CatalogPagingButton
            direction="next"
            className={twMerge("paging-next disabled:opacity-50", pageSlug1 === "intro" && "active")}
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 80 80">
